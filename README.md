...
![Kraken](kraken.svg)
[ Octopus by julian roman from the Noun Project ](https://thenounproject.com/term/octopus/138993/)

![Squid](squid.svg)
[Octopus by Iconic from the Noun Project](https://thenounproject.com/term/octopus/703256/)

Scholars' Lab Makerspace Wiki

A Wiki for tracking projects, institutional history, or whatever.
...
- Using and existing GitHub account, simply clone the git repository to your computer.
  - `git clone https://github.com/scholarslab/makerwiki.git`
- Now you can edit any of the Markdown files in the newly created 'makerwiki' folder.
...
## Modify three files in 'server-configs' folder
...
- These are just the lines that need to be changed.
  ...
  host: http://makerwiki.scholarslab.org
...
- Using version 2 of Docker Compose file because our server's docker-compose version can only take up to version 2
  ...
  - **wikijs section**
    ...
    - 'volumes' basically mounts the local/host file 'config.yml' into the path in the Docker container.
      - For local development, make sure you have the folder '/var/log/www/makerwiki' created already, or delete that line.

### .env
- Create an `.env` file for environment variables

  ```
  GIT_USER_NAME=slabmakerspace
  GIT_USER_PASS=github_user_pass
  GIT_REPO_URL=https://github.com/scholarslab/makerwiki.git
  WIKI_ADMIN_EMAIL=scholars.lab@gmail.com
  ```
...
- Apache http config:
  ...
  ```
  <VirtualHost *:80>
      ServerName makerwiki.scholarslab.org
...
# Access the website
- For production, the website is accessible at http://makerwiki.scholarslab.org
