...
It took many tries to get the design just right, as can be witnessed by the number attached to the end of the files, around 5-6. Some of the earlier versions were garbage attempts that I just threw away as it would be faster to start from scratch than try to fix the model. They also mark the progression from Single Piece, to Better Single Piece, to Two Piece designs, and all the points in the middle. However, the most important thing I want to emphasize in this write-up is that it takes that many tries to get things right. Through each iteration, I learned a new idea, technique, or tool that made the next iteration that much faster. Early on, I had a basic mock-up of an iPhone that I would try to reference within the designs. This was a bad practice that I eventually realized I did not need. These early designs also featured an impractically long list of "features" that could be heavily condensed with the experience I have now. Given the correct measurements, it would only take me probably an hour to completely recreate the design from scratch. The skill I learned was to know when to start over with a design and how to apply that knowledge in order to be more efficient. 