...
//increasing in brightness and half of them to be decreasing.
...
void changeBrightness(int group){
    ...
    } else { //If it is in the dimming phase
      switch(cS){
        case 1: //If it's RED
            leds[group].r = leds[group].r - 5;
          break;
    ...
    if(leds[group].r >250){ upDownState[group] = false;}
    else if(leds[group].r < 6){ upDownState[group] = true; colorState[group] = (int)random(1, 4); }
}
